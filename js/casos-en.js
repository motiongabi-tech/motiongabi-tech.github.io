/* CASOS en inglés — misma estructura que casos.js, solo cambian los textos */
const CASOS_EN = {
 "la-besneta": {
  "titulo": "La Besnéta",
  "hero": {
   "bg": "#FCF3EB",
   "ink": "#743424",
   "titulo": "La Besnéta",
   "sub": [
    "Branding, packaging, photography, web and email design for an artisan cookie dough brand."
   ],
   "pills": [
    [
     "BRANDING",
     "#EF7DB1",
     "#FCF3EB"
    ],
    [
     "PACKAGING",
     "#E64D14",
     "#FCF3EB"
    ],
    [
     "PHOTOGRAPHY",
     "#AFC673",
     "#FCF3EB"
    ],
    [
     "WEB",
     "#743424",
     "#FCF3EB"
    ],
    [
     "EMAIL",
     "#EF7DB1",
     "#FCF3EB"
    ],
    [
     "AI",
     "#E64D14",
     "#FCF3EB"
    ]
   ],
   "meta": "2025 — 2026 · AT STORIA CONTENIDOS",
   "patron": "#EF7DB1",
   "mascota": true
  },
  "marquee": {
   "bg": "#FCF3EB",
   "banda": "#EF7DB1",
   "ink": "#743424",
   "sep": "✦",
   "voces": [
    "Brand identity",
    "Packaging",
    "Photography",
    "Web",
    "Email"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#FFFFFF",
   "ink": "#2B35E8",
   "ep": "THE BRIEF",
   "epInk": "#2B35E8",
   "tit": [
    "Shaping a brand that starts with a family recipe."
   ],
   "txt": [
    "La Besnéta makes artisan, 100% plant-based frozen cookie dough."
   ],
   "meta": [
    [
     "CLIENT",
     "La Besnéta · Barcelona"
    ],
    [
     "STUDIO",
     "Storia Contenidos"
    ],
    [
     "MY ROLE",
     "Branding · Illustration · Packaging · Photography & retouching · Web design · Email design"
    ],
    [
     "TOOLS",
     "Figma · Illustrator · Photoshop · After Effects · Camera · Generative AI · Code"
    ],
    [
     "LIVE",
     "labesneta.com"
    ]
   ],
   "despues": false
  },
  "secciones": [
   {
    "ep": "01 / THE BRAND",
    "epInk": "rgba(116,52,36,.75)",
    "bg": "#EF7DB1",
    "ink": "#FCF3EB",
    "tit": [
     "The logo is a girl with a giant cookie"
    ],
    "bloques": [
     {
      "tipo": "wordmark",
      "color": "#FCF3EB"
     },
     {
      "tipo": "rejilla",
      "cols": 2,
      "piezas": [
       {
        "logo": true,
        "bg": "#FCF3EB",
        "color": "#743424",
        "pie": "POSITIVE · COCOA LOGO ON CREAM"
       },
       {
        "logo": true,
        "bg": "#1a1a1a",
        "color": "#FCF3EB",
        "pie": "NEGATIVE · CREAM LOGO ON BLACK"
       }
      ]
     },
     {
      "tipo": "rejilla",
      "cols": 5,
      "alto": 164,
      "piezas": [
       {
        "logo": true,
        "bg": "#E64D14",
        "color": "#FCF3EB"
       },
       {
        "logo": true,
        "bg": "#AFC673",
        "color": "#743424"
       },
       {
        "logo": true,
        "bg": "#743424",
        "color": "#FCF3EB"
       },
       {
        "logo": true,
        "bg": "#FCF3EB",
        "color": "#EF7DB1"
       },
       {
        "logo": true,
        "bg": "#EF7DB1",
        "color": "#743424"
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / COLOUR PALETTE & TYPOGRAPHY",
    "epInk": "#E64D14",
    "bg": "#FCF3EB",
    "ink": "#743424",
    "tit": [
     "Each flavour has its own colour"
    ],
    "bloques": [
     {
      "tipo": "swatches",
      "piezas": [
       [
        "Pink",
        "#EF7DB1",
        "BRAND"
       ],
       [
        "Orange",
        "#E64D14",
        "DOUBLE CHOCOLATE"
       ],
       [
        "Pistachio",
        "#AFC673",
        "PISTACHIO"
       ],
       [
        "Cocoa",
        "#743424",
        "TEXT & BACKGROUND"
       ],
       [
        "Cream",
        "#FCF3EB",
        "AIR"
       ]
      ]
     },
     {
      "tipo": "specimen",
      "piezas": [
       [
        "Modak",
        "KEYWORDS"
       ],
       [
        "Baloo 2",
        "HEADINGS"
       ],
       [
        "Plus Jakarta Sans",
        "BODY TEXT"
       ]
      ]
     }
    ]
   },
   {
    "ep": "03 / PACKAGING",
    "epInk": "rgba(252,243,235,.8)",
    "bg": "#E64D14",
    "ink": "#FCF3EB",
    "tit": [
     "Kraft on the outside, colour on the inside"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/la-besneta/04-01.jpg",
          "ar": 1
         },
         {
          "apilado": [
           {
            "src": "img/caso/la-besneta/04-02.jpg",
            "ar": 2.0673076923076925
           },
           {
            "src": "img/caso/la-besneta/04-03.jpg",
            "ar": 2.0739549839228295
           }
          ]
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/la-besneta/04-04.jpg",
          "ar": 0.899290758498171
         },
         {
          "src": "img/caso/la-besneta/04-05.jpg",
          "ar": 0.899290758498171
         },
         {
          "src": "img/caso/la-besneta/04-06.jpg",
          "ar": 0.899290758498171
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "04 / PHOTOGRAPHY",
    "epInk": "#EF7DB1",
    "bg": "#743424",
    "ink": "#FCF3EB",
    "sangrado": true,
    "tit": [
     "Hard light, colour and the cookie as the star"
    ],
    "bloques": [
     {
      "tipo": "mosaico",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "ar": 0.7741935483870968,
        "piezas": [
         {
          "src": "img/caso/la-besneta/05-01.jpg"
         },
         {
          "src": "img/caso/la-besneta/05-02.jpg"
         },
         {
          "src": "img/caso/la-besneta/05-03.jpg"
         }
        ]
       },
       {
        "cols": "576fr 864fr",
        "ar": null,
        "piezas": [
         {
          "src": "img/caso/la-besneta/05-04.jpg",
          "ar": 1.2521739130434784
         },
         {
          "src": "img/caso/la-besneta/05-05.jpg",
          "ar": 1.8782608695652174
         }
        ]
       },
       {
        "cols": "864fr 576fr",
        "ar": null,
        "piezas": [
         {
          "src": "img/caso/la-besneta/05-06.jpg",
          "ar": 1.8782608695652174
         },
         {
          "src": "img/caso/la-besneta/05-07.jpg",
          "ar": 1.2521739130434784
         }
        ]
       }
      ]
     },
     {
      "tipo": "pie2",
      "izq": "PRODUCTION, CAMERA & RETOUCHING · GABI PÉREZ",
      "der": "PHOTOSHOP · SELECTION FROM 100+ SHOTS"
     }
    ]
   },
   {
    "ep": "05 / THE WEBSITE",
    "epInk": "#E64D14",
    "bg": "#FCF3EB",
    "ink": "#743424",
    "tit": [
     "The same photo, but in motion"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/la-besneta/06-01.jpg",
          "insignia": "PHOTO",
          "ar": 1
         },
         {
          "video": "img/caso/video/besneta-header.mp4",
          "poster": "img/caso/la-besneta/06-02.jpg",
          "insignia": "AI",
          "esVideo": true,
          "ar": 1
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/la-besneta/06-03.jpg",
          "ar": 1.0063491821289061
         },
         {
          "src": "img/caso/la-besneta/06-04.jpg",
          "ar": 1.0063491821289061
         },
         {
          "src": "img/caso/la-besneta/06-05.jpg",
          "ar": 1.0063491821289061
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "06 / EMAIL",
    "epInk": "#743424",
    "bg": "#EF7DB1",
    "ink": "#FCF3EB",
    "tit": [
     "The brand has to work in email too"
    ],
    "bloques": [
     {
      "tipo": "correos",
      "piezas": [
       {
        "src": "img/caso/la-besneta/07-01.jpg"
       },
       {
        "src": "img/caso/la-besneta/07-03.jpg"
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#743424",
   "ink": "#FCF3EB",
   "patron": "#FCF3EB",
   "logo": "besneta",
   "mascota": true,
   "tools": [
    "FIGMA",
    "ILLUSTRATOR",
    "PHOTOSHOP",
    "AFTER EFFECTS",
    "PHOTOGRAPHY",
    "GENERATIVE AI",
    "CODE"
   ],
   "enlace": {
    "txt": "labesneta.com ↗",
    "url": "https://labesneta.com"
   }
  },
  "siguiente": "canva-icons"
 },
 "canva-icons": {
  "titulo": "Canva",
  "hero": {
   "bg": "#992BFF",
   "bgGrad": "linear-gradient(180deg,#992BFF 0%,#5A32FA 27.5%,#13A3B5 50.1%,#93E8F6 68.2%,#F1EBFF 84.1%,#FFFFFF 98.1%)",
   "ink": "#FFFFFF",
   "titulo": "Canva",
   "sub": [
    "Icons for the login screen. Two markets, two cultures, the same language."
   ],
   "pills": [
    [
     "ICON DESIGN",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ],
    [
     "ILLUSTRATION",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ],
    [
     "SPAIN",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ],
    [
     "UNITED KINGDOM",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ]
   ],
   "meta": "2026 · CLIENT: CANVA",
   "flotantes": [
    {
     "src": "img/caso/canva/HERO-01.png",
     "w": 300
    },
    {
     "src": "img/caso/canva/HERO-02.png",
     "w": 240
    },
    {
     "src": "img/caso/canva/HERO-03.png",
     "w": 260
    }
   ]
  },
  "marquee": {
   "bg": "#FFFFFF",
   "banda": "#7D2AE7",
   "ink": "#FFFFFF",
   "sep": "✦",
   "voces": [
    "Icon design",
    "Spain",
    "United Kingdom",
    "Illustration"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#FFFFFF",
   "ink": "#0E1318",
   "ep": "THE BRIEF",
   "epInk": "#7D2AE7",
   "tit": [
    "An icon has very little space to say a lot."
   ],
   "txt": [
    "Canva needed local icon sets for its login screen."
   ],
   "meta": [
    [
     "CLIENT",
     "Canva"
    ],
    [
     "YEAR",
     "2026"
    ],
    [
     "MY ROLE",
     "Icon design · Illustration"
    ],
    [
     "WHERE",
     "Login screen, Spain and UK"
    ],
    [
     "TOOLS",
     "Illustrator · Figma"
    ]
   ],
   "despues": false
  },
  "secciones": [
   {
    "ep": "01 / THE SPAIN SET",
    "epInk": "#7D2AE7",
    "bg": "#FFFFFF",
    "ink": "#0E1318",
    "tit": [
     "The World Cup, in just a few strokes"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/canva/01-01.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/canva/01-02.jpg",
          "ar": 1
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / IN PRODUCTION",
    "epInk": "#00C4CC",
    "bg": "#0E1318",
    "ink": "#FFFFFF",
    "tit": [
     "This is what they look like once they're in context"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/canva/02-01.jpg",
          "pie": "CANVA LOGIN SCREEN · SPAIN",
          "ar": 1.7777777777777777
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / INSIDE THE INTERFACE",
    "epInk": "#00C4CC",
    "bg": "#7D2AE7",
    "ink": "#FFFFFF",
    "tit": [
     "Some icons also communicate a function"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/canva/03-01.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/canva/03-02.jpg",
          "ar": 1
         }
        ]
       },
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/canva/03-03.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/canva/03-04.jpg",
          "ar": 1
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "04 / THE UK SET",
    "epInk": "#7D2AE7",
    "bg": "#F1EBFF",
    "ink": "#0E1318",
    "tit": [
     "Recognisable without turning it into a souvenir"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "tarjeta": "linear-gradient(180deg,#992BFF 0%,#5A32FA 27.5%,#13A3B5 50.1%,#93E8F6 68.2%,#F1EBFF 84.1%,#FFFFFF 98.1%)",
        "piezas": [
         {
          "src": "img/caso/canva/04-01.png",
          "ar": 0.8453333129882812
         },
         {
          "src": "img/caso/canva/04-02.png",
          "ar": 0.8453333129882812
         },
         {
          "src": "img/caso/canva/04-03.png",
          "ar": 0.8453333129882812
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#992BFF",
   "bgGrad": "linear-gradient(180deg,#992BFF 0%,#5A32FA 27.5%,#13A3B5 50.1%,#93E8F6 68.2%,#F1EBFF 84.1%,#FFFFFF 98.1%)",
   "ink": "#FFFFFF",
   "logo": "canva",
   "tools": [
    "ILLUSTRATOR",
    "FIGMA",
    "ICON DESIGN",
    "ILLUSTRATION"
   ]
  },
  "siguiente": "miniso"
 },
 "mahou": {
  "titulo": "Mahou",
  "hero": {
   "bg": "#c70e2d",
   "ink": "#fff6e8",
   "tituloInk": "#fff6e8",
   "titulo": "Mahou",
   "sub": [
    "El Patio de Mahou: a graphic universe for summer in Madrid."
   ],
   "pills": [
    [
     "ILLUSTRATION",
     "#fbc84b",
     "#1d1d1b"
    ],
    [
     "ANIMATION",
     "#fff6e8",
     "#c70e2d"
    ],
    [
     "MOTION",
     "#fbc84b",
     "#1d1d1b"
    ],
    [
     "SPACE",
     "#fff6e8",
     "#c70e2d"
    ]
   ],
   "meta": "2021 — 2023 · CLIENT: MAHOU",
   "imagen": {
    "src": "img/caso/mahou/HERO-01.jpg",
    "ar": 1.75
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#fbc84b",
   "ink": "#be051d",
   "sep": "★",
   "voces": [
    "El Patio",
    "Illustration",
    "Motion"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#1d1d1b",
   "ep": "THE BRIEF",
   "epInk": "#c70e2d",
   "tit": [
    "I didn't want to draw the patio. I wanted to draw what it feels like to be there."
   ],
   "txt": [
    "El Patio Mahou was a place to eat, listen to music and spend the afternoon."
   ],
   "meta": [
    [
     "CLIENT",
     "Mahou"
    ],
    [
     "YEARS",
     "2021 — 2023"
    ],
    [
     "MY ROLE",
     "Illustration · Animation · Motion"
    ],
    [
     "FORMATS",
     "Mural · posters · table linen · social media"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / THE GRAPHIC UNIVERSE",
    "epInk": "#c70e2d",
    "bg": "#fff6e8",
    "ink": "#1d1d1b",
    "tit": [
     "A collection of pieces, not a single poster"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "860fr 430fr",
        "piezas": [
         {
          "src": "img/caso/mahou/01-01.jpg",
          "ar": 1
         },
         {
          "apilado": [
           {
            "src": "img/caso/mahou/01-02.jpg",
            "ar": 1.0263
           },
           {
            "src": "img/caso/mahou/01-03.jpg",
            "ar": 1.0263
           }
          ]
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / THE SPACE",
    "epInk": "#fbc84b",
    "bg": "#c70e2d",
    "ink": "#fff6e8",
    "tit": [
     "The graphics left the screen and ended up in the patio"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/mahou/02-01.jpg",
          "ar": 1.5
         },
         {
          "src": "img/caso/mahou/02-02.jpg",
          "ar": 1.5
         }
        ]
       },
       {
        "cols": "430fr 860fr",
        "piezas": [
         {
          "src": "img/caso/mahou/02-03.jpg",
          "ar": 0.7679
         },
         {
          "src": "img/caso/mahou/02-04.jpg",
          "ar": 1.5357
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / ANIMATION",
    "epInk": "#fbc84b",
    "bg": "#1d1d1b",
    "ink": "#fff6e8",
    "tit": [],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "video": "img/caso/video/mahou-loop-01.mp4",
          "poster": "img/caso/mahou/03-01.jpg",
          "src": "img/caso/mahou/03-01.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 1
         },
         {
          "video": "img/caso/video/mahou-loop-02.mp4",
          "poster": "img/caso/mahou/03-02.jpg",
          "src": "img/caso/mahou/03-02.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 1
         },
         {
          "video": "img/caso/video/mahou-loop-03.mp4",
          "poster": "img/caso/mahou/03-03.jpg",
          "src": "img/caso/mahou/03-03.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 1
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "video": "img/caso/video/mahou-loop-04.mp4",
          "poster": "img/caso/mahou/03-04.jpg",
          "src": "img/caso/mahou/03-04.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 1
         },
         {
          "video": "img/caso/video/mahou-loop-06.mp4",
          "poster": "img/caso/mahou/03-05.jpg",
          "src": "img/caso/mahou/03-05.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 1
         },
         {
          "video": "img/caso/video/mahou-loop-08.mp4",
          "poster": "img/caso/mahou/03-06.jpg",
          "src": "img/caso/mahou/03-06.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 1
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#be051d",
   "ink": "#fff6e8",
   "tools": [
    "ILLUSTRATOR",
    "AFTER EFFECTS",
    "ILLUSTRATION",
    "MOTION"
   ],
   "rotulo": "Mahou",
   "rotuloInk": "#fff6e8"
  },
  "siguiente": "miniso"
 },
 "miniso": {
  "titulo": "Miniso",
  "hero": {
   "bg": "#2c55ff",
   "ink": "#ffffff",
   "tituloInk": "#ffffff",
   "titulo": "Miniso",
   "sub": [
    "Seasonal campaigns and daily content for the feed. A whole year of brand."
   ],
   "pills": [
    [
     "ILLUSTRATION",
     "#ffc300",
     "#101322"
    ],
    [
     "DESIGN",
     "#ffffff",
     "#1e42e0"
    ],
    [
     "ANIMATION",
     "#ffc300",
     "#101322"
    ],
    [
     "SOCIAL",
     "#ffffff",
     "#1e42e0"
    ],
    [
     "3D MODELLING",
     "#ffc300",
     "#101322"
    ]
   ],
   "meta": "2023 · CLIENT: MINISO",
   "imagen": {
    "src": "img/caso/miniso/HERO-01.jpg",
    "ar": 1.52
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#ffc300",
   "ink": "#1e42e0",
   "sep": "●",
   "voces": [
    "Summer",
    "Father's Day",
    "Always-on",
    "3D"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#101322",
   "ep": "THE BRIEF",
   "epInk": "#1e42e0",
   "tit": [
    "A campaign can make noise. Daily content has to hold the brand up."
   ],
   "txt": [
    "For Miniso, I worked on seasonal campaigns as well as the content that appears week after week on the feed."
   ],
   "meta": [
    [
     "CLIENT",
     "Miniso"
    ],
    [
     "YEAR",
     "2023"
    ],
    [
     "MY ROLE",
     "Illustration · Design · Animation · 3D modelling"
    ],
    [
     "THREE AREAS",
     "Summer campaign · Father's Day · always-on content"
    ],
    [
     "FORMATS",
     "Posters · 9:16 for stories · loop animation"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / SUMMER CAMPAIGN",
    "epInk": "#ffc300",
    "bg": "#2c55ff",
    "ink": "#ffffff",
    "tit": [
     "Summer, without turning down the volume"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/miniso/01-01.jpg",
          "ar": 1.5274
         }
        ]
       },
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/miniso/01-02.jpg",
          "ar": 0.7523
         }
        ]
       },
       {
        "cols": "430fr 860fr",
        "piezas": [
         {
          "src": "img/caso/miniso/01-03.jpg",
          "ar": 0.7452
         },
         {
          "src": "img/caso/miniso/01-04.jpg",
          "ar": 1.4905
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / FATHER'S DAY",
    "epInk": "#ffffff",
    "bg": "#d16376",
    "ink": "#ffffff",
    "tit": [],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/miniso/02-01.jpg",
          "pie": "CAMPAIGN GRAPHIC APPLIED TO SHOPPING BAG",
          "ar": 1.7826
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / SOCIAL MEDIA",
    "epInk": "#1e42e0",
    "bg": "#f2f4fb",
    "ink": "#101322",
    "tit": [
     "Content for social media"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "video": "img/caso/video/miniso-story-01.mp4",
          "poster": "img/caso/miniso/03-01.jpg",
          "src": "img/caso/miniso/03-01.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 0.5626
         },
         {
          "video": "img/caso/video/miniso-story-02.mp4",
          "poster": "img/caso/miniso/03-02.jpg",
          "src": "img/caso/miniso/03-02.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 0.5626
         },
         {
          "video": "img/caso/video/miniso-story-03.mp4",
          "poster": "img/caso/miniso/03-03.jpg",
          "src": "img/caso/miniso/03-03.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 0.5626
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "video": "img/caso/video/miniso-story-04.mp4",
          "poster": "img/caso/miniso/03-04.jpg",
          "src": "img/caso/miniso/03-04.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 0.5626
         },
         {
          "video": "img/caso/video/miniso-story-05.mp4",
          "poster": "img/caso/miniso/03-05.jpg",
          "src": "img/caso/miniso/03-05.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 0.5626
         },
         {
          "video": "img/caso/video/miniso-story-06.mp4",
          "poster": "img/caso/miniso/03-06.jpg",
          "src": "img/caso/miniso/03-06.jpg",
          "esVideo": true,
          "insignia": "LOOP",
          "ar": 0.5626
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#1e42e0",
   "ink": "#ffffff",
   "tools": [
    "ILLUSTRATOR",
    "AFTER EFFECTS",
    "3D",
    "SOCIAL"
   ],
   "rotulo": "Miniso",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "corny"
 },
 "corny": {
  "titulo": "Corny",
  "hero": {
   "bg": "#f07e22",
   "ink": "#ffffff",
   "tituloInk": "#ffffff",
   "titulo": "Corny",
   "sub": [
    "AI-assisted photo retouching: seven before-and-after pairs."
   ],
   "pills": [
    [
     "RETOUCHING",
     "#16357f",
     "#ffffff"
    ],
    [
     "PHOTOSHOP",
     "#ffffff",
     "#d05319"
    ],
    [
     "AI EDITING",
     "#16357f",
     "#ffffff"
    ],
    [
     "PACKSHOT",
     "#ffffff",
     "#d05319"
    ]
   ],
   "meta": "CLIENT: CORNY · AT PLAYROOM",
   "imagen": {
    "src": "img/caso/corny/HERO-01.jpg",
    "ar": 0.915
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#16357f",
   "ink": "#ffffff",
   "sep": "→",
   "voces": [
    "Before",
    "After"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#14100c",
   "ep": "THE BRIEF",
   "epInk": "#d05319",
   "tit": [
    "Improving the photo without changing what's inside it."
   ],
   "txt": [
    "I retouched seven Corny product images with the help of AI."
   ],
   "meta": [
    [
     "CLIENT",
     "Corny"
    ],
    [
     "STUDIO",
     "PLAYROOM"
    ],
    [
     "MY ROLE",
     "Photoshop retouching · AI editing"
    ],
    [
     "DELIVERY",
     "7 product images"
    ],
    [
     "USE",
     "Packshot and brand content"
    ]
   ],
   "despues": true
  },
  "secciones": [
   {
    "ep": "01 / BEFORE & AFTER",
    "epInk": "#d05319",
    "bg": "#fbf3ea",
    "ink": "#14100c",
    "tit": [
     "Seven images, seven different problems"
    ],
    "bloques": [
     {
      "tipo": "pares",
      "piezas": [
       {
        "n": "PAIR 02",
        "antes": "img/caso/corny/01-01.jpg",
        "despues": "img/caso/corny/01-02.jpg"
       },
       {
        "n": "PAIR 01",
        "antes": "img/caso/corny/01-03.jpg",
        "despues": "img/caso/corny/01-04.jpg"
       },
       {
        "n": "PAIR 03",
        "antes": "img/caso/corny/01-05.jpg",
        "despues": "img/caso/corny/01-06.jpg"
       },
       {
        "n": "PAIR 04",
        "antes": "img/caso/corny/01-07.jpg",
        "despues": "img/caso/corny/01-08.jpg"
       },
       {
        "n": "PAIR 05",
        "antes": "img/caso/corny/01-09.jpg",
        "despues": "img/caso/corny/01-10.jpg"
       },
       {
        "n": "PAIR 06",
        "antes": "img/caso/corny/01-11.jpg",
        "despues": "img/caso/corny/01-12.jpg"
       },
       {
        "n": "PAIR 07",
        "antes": "img/caso/corny/01-13.jpg",
        "despues": "img/caso/corny/01-14.jpg"
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#16357f",
   "ink": "#ffffff",
   "tools": [
    "PHOTOSHOP",
    "AI EDITING",
    "PRODUCT RETOUCHING"
   ],
   "rotulo": "Corny",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "victoria"
 },
 "victoria": {
  "titulo": "Victoria",
  "hero": {
   "bg": "#83d3c8",
   "ink": "#20302e",
   "tituloInk": "#ffffff",
   "titulo": "Victoria",
   "sub": [
    "A hand-drawn summer spot."
   ],
   "pills": [
    [
     "MOTION GRAPHICS",
     "#d6403c",
     "#ffffff"
    ],
    [
     "ILLUSTRATION",
     "#ffffff",
     "#5dbbae"
    ],
    [
     "DIRECTION",
     "#f0b93e",
     "#20302e"
    ],
    [
     "SOCIAL",
     "#ffffff",
     "#5dbbae"
    ]
   ],
   "meta": "2021 · VICTORIA CALZADO · IED FINAL PROJECT",
   "imagen": {
    "src": "img/caso/victoria/HERO-01.jpg",
    "ar": 1
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#f0b93e",
   "ink": "#20302e",
   "sep": "✦",
   "voces": [
    "#doblevictoria",
    "Summer",
    "Illustration",
    "Motion"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#20302e",
   "ep": "THE BRIEF",
   "epInk": "#5dbbae",
   "tit": [
    "Telling a whole summer in fifteen seconds."
   ],
   "txt": [
    "Final Motion Graphics project at IED."
   ],
   "meta": [
    [
     "CLIENT",
     "Victoria Calzado"
    ],
    [
     "YEAR",
     "2021"
    ],
    [
     "CONTEXT",
     "Motion Graphics final project · IED"
    ],
    [
     "MY ROLE",
     "Motion graphics · Design · Illustration"
    ],
    [
     "DURATION",
     "15 seconds"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / THE SPOT",
    "epInk": "#f0b93e",
    "bg": "#20302e",
    "ink": "#ffffff",
    "tit": [],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "video": "img/caso/video/victoria-spot.mp4",
          "poster": "img/caso/victoria/02-05.jpg",
          "src": "img/caso/victoria/02-05.jpg",
          "esVideo": true,
          "insignia": "VIDEO · 15S",
          "ar": 1.7778
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / STORYBOARD",
    "epInk": "#5dbbae",
    "bg": "#ffe8dc",
    "ink": "#20302e",
    "tit": [
     "Storyboard"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/victoria/02-01.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-02.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-03.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-04.jpg",
          "ar": 1.7784
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/victoria/02-05.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-06.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-07.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-08.jpg",
          "ar": 1.7784
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/victoria/02-09.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-10.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-11.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-12.jpg",
          "ar": 1.7784
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/victoria/02-13.jpg",
          "ar": 1.7784
         },
         {
          "src": "img/caso/victoria/02-14.jpg",
          "ar": 1.7784
         }
        ]
       }
      ]
     },
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/victoria/03-01.jpg",
          "ar": 1.7967
         },
         {
          "src": "img/caso/victoria/03-02.jpg",
          "ar": 1.7967
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#83d3c8",
   "ink": "#ffffff",
   "tools": [
    "AFTER EFFECTS",
    "ILLUSTRATION",
    "MOTION GRAPHICS"
   ],
   "rotulo": "Victoria",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "bumbba"
 },
 "bumbba": {
  "titulo": "Bumbba",
  "hero": {
   "bg": "#f4e5d3",
   "ink": "rgba(20,19,16,.8)",
   "tituloInk": "#141310",
   "titulo": "Bumbba",
   "sub": [
    "An assembly booklet for a sofa that doesn't need assembling."
   ],
   "pills": [
    [
     "EDITORIAL LAYOUT",
     "#aaa130",
     "#ffffff"
    ],
    [
     "ILLUSTRATION",
     "#f5b0cd",
     "#141310"
    ],
    [
     "3D",
     "#b7d7e4",
     "#141310"
    ],
    [
     "EMAIL",
     "#aaa130",
     "#ffffff"
    ]
   ],
   "meta": "2025 · CLIENT: BUMBBA · AT STORIA CONTENIDOS",
   "imagen": {
    "src": "img/caso/bumbba/01.jpg",
    "ar": 1
   }
  },
  "marquee": {
   "bg": "#b7d7e4",
   "banda": "#f5b0cd",
   "ink": "#141310",
   "sep": "●",
   "voces": [
    "Imagine",
    "Build",
    "Enjoy"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#b7d7e4",
   "ink": "#141310",
   "ep": "THE BRIEF",
   "epInk": "#aaa130",
   "tit": [
    "An assembly manual for something that doesn't need assembling?"
   ],
   "txt": [
    "Bumbba makes modular sofas that don't need assembling."
   ],
   "meta": [
    [
     "CLIENT",
     "Bumbba — modular sofas"
    ],
    [
     "STUDIO",
     "Storia Contenidos"
    ],
    [
     "YEAR",
     "2025"
    ],
    [
     "MY ROLE",
     "Editorial layout · Illustration · 3D · Generative AI"
    ],
    [
     "TWO DELIVERABLES",
     "Printed booklet · illustrations for the website"
    ],
    [
     "TOOL",
     "Adobe Neo Beta for the 3D pieces"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / THE BOOKLET",
    "epInk": "#aaa130",
    "bg": "#f4e5d3",
    "ink": "#141310",
    "tit": [
     "Assembly instructions for a sofa with no assembly"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "860fr 452fr",
        "piezas": [
         {
          "src": "img/caso/bumbba/02.png",
          "ar": 1.2798
         }
        ]
       },
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/bumbba/03.jpg",
          "ar": 1.4909
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / 3D ILLUSTRATION",
    "epInk": "#141310",
    "bg": "#d8eaef",
    "ink": "#141310",
    "tit": [
     "One pouf, many lives"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "repeat(3,1fr)",
        "piezas": [
         {
          "src": "img/caso/bumbba/04.jpg",
          "ar": 1.281
         },
         {
          "src": "img/caso/bumbba/05.jpg",
          "ar": 1.281
         },
         {
          "src": "img/caso/bumbba/06.jpg",
          "ar": 1.281
         }
        ]
       },
       {
        "cols": "repeat(3,1fr)",
        "piezas": [
         {
          "src": "img/caso/bumbba/07.jpg",
          "ar": 1.281
         },
         {
          "src": "img/caso/bumbba/08.jpg",
          "ar": 1.281
         },
         {
          "src": "img/caso/bumbba/09.jpg",
          "ar": 1.281
         }
        ]
       },
       {
        "cols": "repeat(3,1fr)",
        "piezas": [
         {
          "src": "img/caso/bumbba/10.jpg",
          "ar": 1.281
         },
         {
          "ar": 1.281,
          "insignia": "LOOP",
          "poster": "img/caso/bumbba/11.jpg",
          "video": "img/caso/video/bumbba-pouf.mp4"
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / ON THE WEBSITE",
    "epInk": "#aaa130",
    "bg": "#ffffff",
    "ink": "#141310",
    "tit": [
     "It's not just a sofa. Neither is the pouf."
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/bumbba/12.jpg",
          "pie": "PRODUCT PAGE ON BUMBBA.ES",
          "ar": 1.8716
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "04 / ARM BOOKLET",
    "epInk": "#fbc30c",
    "bg": "#ea501c",
    "ink": "#ede6da",
    "tit": [
     "An arm can be much more than an arm"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "640fr 646fr",
        "piezas": [
         {
          "src": "img/caso/bumbba/13.jpg",
          "ar": 0.7033
         },
         {
          "panel": {
           "bg": "#fbc30c",
           "ink": "#141310",
           "tit": [
            "“Nothing has just one purpose. Not even the arms.”"
           ]
          }
         }
        ]
       },
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/bumbba/14.jpg",
          "ar": 1.4032
         }
        ]
       },
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/bumbba/15.jpg",
          "ar": 3.3215
         }
        ]
       },
       {
        "cols": "640fr 646fr",
        "piezas": [
         {
          "src": "img/caso/bumbba/16.jpg",
          "ar": 0.7033
         },
         {
          "src": "img/caso/bumbba/17.jpg",
          "ar": 1.2568
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "05 / EMAIL",
    "epInk": "#ea501c",
    "bg": "#b7d7e4",
    "ink": "#141310",
    "tit": [
     "Email design"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "repeat(5,1fr)",
        "piezas": [
         {
          "src": "img/caso/bumbba/18.jpg",
          "ar": 0.3658
         },
         {
          "src": "img/caso/bumbba/19.jpg",
          "ar": 0.2132
         },
         {
          "src": "img/caso/bumbba/20.jpg",
          "ar": 0.212
         },
         {
          "src": "img/caso/bumbba/21.jpg",
          "ar": 0.2559
         },
         {
          "src": "img/caso/bumbba/22.jpg",
          "ar": 0.2357
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#141310",
   "ink": "#f4e5d3",
   "tools": [
    "INDESIGN",
    "ILLUSTRATOR",
    "ADOBE NEO BETA",
    "GENERATIVE AI"
   ],
   "rotulo": "Bumbba",
   "rotuloInk": "#f4e5d3"
  },
  "siguiente": "capri-sun"
 },
 "capri-sun": {
  "titulo": "Capri-Sun",
  "hero": {
   "bg": "#003c8c",
   "bgGrad": "linear-gradient(180deg,#003c8c 0%,#0173cb 55%,#01b3e7 100%)",
   "ink": "rgba(255,255,255,.95)",
   "tituloInk": "#ffffff",
   "titulo": "Capri-Sun",
   "sub": [
    "Launch of the Cola flavour in the UK. Three loops that make it clear at a glance."
   ],
   "pills": [
    [
     "DESIGN",
     "#ffffff",
     "#003c8c"
    ],
    [
     "ANIMATION",
     "#6b3218",
     "#ffffff"
    ],
    [
     "SOCIAL 9:16",
     "#ffffff",
     "#003c8c"
    ],
    [
     "UK",
     "#6b3218",
     "#ffffff"
    ]
   ],
   "meta": "2024 · CLIENT: CAPRI-SUN",
   "imagen": {
    "src": "img/caso/capri-sun/01.jpg",
    "ar": 0.5628
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#6b3218",
   "ink": "#ffffff",
   "sep": "●",
   "voces": [
    "New flavour",
    "Cola",
    "UK"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE BRIEF",
   "epInk": "#003c8c",
   "tit": [
    "Make it clear as soon as it appears on screen."
   ],
   "txt": [
    "Social content for the launch of Capri-Sun's Cola flavour in the UK."
   ],
   "meta": [
    [
     "CLIENT",
     "Capri-Sun"
    ],
    [
     "YEAR",
     "2024"
    ],
    [
     "MARKET",
     "United Kingdom"
    ],
    [
     "MY ROLE",
     "Design · Animation"
    ],
    [
     "DELIVERY",
     "3 pieces in 9:16 for social media"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / THE FAMILY",
    "epInk": "#003c8c",
    "bg": "#eff4fa",
    "ink": "#0a1a2e",
    "tit": [
     "Three pieces, one campaign"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "repeat(3,1fr)",
        "piezas": [
         {
          "ar": 0.5628,
          "insignia": "LOOP",
          "poster": "img/caso/capri-sun/01.jpg",
          "video": "img/caso/video/capri-loop-01.mp4"
         },
         {
          "ar": 0.5628,
          "insignia": "LOOP",
          "poster": "img/caso/capri-sun/02.jpg",
          "video": "img/caso/video/capri-loop-02.mp4"
         },
         {
          "ar": 0.5628,
          "insignia": "LOOP",
          "poster": "img/caso/capri-sun/03.jpg",
          "video": "img/caso/video/capri-loop-03.mp4"
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / FRAMES",
    "epInk": "#01b3e7",
    "bg": "#0a1a2e",
    "ink": "#ffffff",
    "tit": [],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "repeat(4,1fr)",
        "piezas": [
         {
          "src": "img/caso/capri-sun/04.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/capri-sun/05.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/capri-sun/06.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/capri-sun/07.jpg",
          "ar": 0.5629
         }
        ]
       },
       {
        "cols": "repeat(4,1fr)",
        "piezas": [
         {
          "src": "img/caso/capri-sun/08.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/capri-sun/09.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/capri-sun/10.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/capri-sun/11.jpg",
          "ar": 0.5629
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / MARKET",
    "epInk": "#003c8c",
    "bg": "#ffffff",
    "ink": "#0a1a2e",
    "tit": [
     "One skyline is enough to place it"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "560fr 752fr",
        "piezas": [
         {
          "src": "img/caso/capri-sun/12.jpg",
          "pie": "THE LONDON SKYLINE, IN THE SAME BLUE",
          "ar": 0.5623
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#003c8c",
   "ink": "#ffffff",
   "tools": [
    "AFTER EFFECTS",
    "DESIGN",
    "ANIMATION",
    "SOCIAL"
   ],
   "rotulo": "Capri-Sun",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "hero-solo-bio"
 },
 "hero-solo-bio": {
  "titulo": "Hero Solo Bio",
  "hero": {
   "bg": "#97bee5",
   "bgGrad": "linear-gradient(180deg,#97bee5 0%,#bcd5b9 62%,#d9e083 100%)",
   "ink": "#123b7a",
   "tituloInk": "#ffffff",
   "titulo": "Hero Solo Bio",
   "sub": [
    "Purées, puffs and a collaboration with Bluey. Four social pieces for baby food."
   ],
   "pills": [
    [
     "DESIGN",
     "#ffffff",
     "#123b7a"
    ],
    [
     "ANIMATION",
     "#f2b60e",
     "#123b7a"
    ],
    [
     "STORYBOARD",
     "#ffffff",
     "#123b7a"
    ],
    [
     "SOUND",
     "#f2b60e",
     "#123b7a"
    ]
   ],
   "meta": "CLIENT: HERO · AT PLAYROOM",
   "imagen": {
    "src": "img/caso/hero-solo-bio/01.jpg",
    "ar": 0.8007
   }
  },
  "marquee": {
   "bg": "#f7e5c9",
   "banda": "#35682a",
   "ink": "#f7e5c9",
   "sep": "●",
   "voces": [
    "Solo Bio",
    "Bluey",
    "100% natural"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE BRIEF",
   "epInk": "#003c8c",
   "tit": [
    "The same brand, two very different worlds."
   ],
   "txt": [
    "Social content for Hero Solo Bio, Hero's organic baby food range."
   ],
   "meta": [
    [
     "CLIENT",
     "Hero · Solo Bio"
    ],
    [
     "STUDIO",
     "PLAYROOM"
    ],
    [
     "FORMAT",
     "4:5 for social media"
    ],
    [
     "MY ROLE",
     "Storyboard · Design · Animation · Sound"
    ],
    [
     "DELIVERY",
     "4 animated pieces + 1 spot with audio"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / THE NATURAL SIDE",
    "epInk": "#b7c367",
    "bg": "#f7e5c9",
    "ink": "#35682a",
    "tit": [],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "424fr 424fr 440fr",
        "piezas": [
         {
          "ar": 0.8,
          "poster": "img/caso/hero-solo-bio/02.jpg",
          "video": "img/caso/video/hero-puffs.mp4"
         },
         {
          "ar": 0.8,
          "poster": "img/caso/hero-solo-bio/03.jpg",
          "video": "img/caso/video/hero-cremas.mp4"
         },
         {
          "panel": {
           "bg": "#b7c367",
           "ink": "#ffffff",
           "ep": "THE IDEA",
           "tit": [
            "“100% natural ingredients”"
           ]
          }
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / THE BLUEY WORLD",
    "epInk": "#ffffff",
    "bg": "#97bee5",
    "ink": "#123b7a",
    "tit": [
     "And suddenly, Bluey appears"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "440fr 424fr 424fr",
        "piezas": [
         {
          "panel": {
           "bg": "#123b7a",
           "ink": "#ffffff",
           "ep": "THE THREAD",
           "epInk": "#d9e083",
           "tit": [
            "Another world, the same rhythm"
           ]
          }
         },
         {
          "ar": 0.8,
          "poster": "img/caso/hero-solo-bio/04.jpg",
          "video": "img/caso/video/hero-cacao.mp4"
         },
         {
          "ar": 0.8,
          "poster": "img/caso/hero-solo-bio/05.jpg",
          "video": "img/caso/video/hero-bolsitas.mp4"
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / WITH SOUND",
    "epInk": "#35682a",
    "bg": "#f2b60e",
    "ink": "#35682a",
    "tit": [
     "Sound is part of the edit too"
    ],
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "424fr 862fr",
        "piezas": [
         {
          "ar": 0.8,
          "poster": "img/caso/hero-solo-bio/06.jpg",
          "video": "img/caso/video/hero-spot.mp4"
         },
         {
          "panel": {
           "bg": "#35682a",
           "ink": "#ffffff",
           "ep": "FROM PANEL TO SOUND",
           "epInk": "#d9e083",
           "tit": [
            "Storyboard, design, animation and sound"
           ]
          }
         }
        ]
       }
      ]
     },
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "repeat(5,1fr)",
        "piezas": [
         {
          "src": "img/caso/hero-solo-bio/07.jpg",
          "ar": 0.8
         },
         {
          "src": "img/caso/hero-solo-bio/08.jpg",
          "ar": 0.8
         },
         {
          "src": "img/caso/hero-solo-bio/09.jpg",
          "ar": 0.8
         },
         {
          "src": "img/caso/hero-solo-bio/10.jpg",
          "ar": 0.8
         },
         {
          "src": "img/caso/hero-solo-bio/11.jpg",
          "ar": 0.8
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#35682a",
   "ink": "#d9e083",
   "tools": [
    "AFTER EFFECTS",
    "ILLUSTRATOR",
    "STORYBOARD",
    "SOUND"
   ],
   "rotulo": "Hero Solo Bio",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "miniso"
 },
 "colorbaby": {
  "titulo": "Colorbaby",
  "hero": {
   "bg": "#ffffff",
   "ink": "#0f0f0f",
   "tituloInk": "#e30613",
   "titulo": "Colorbaby",
   "sub": [
    "2023 calendar"
   ],
   "pills": [
    [
     "ILLUSTRATION",
     "#e30613",
     "#ffffff"
    ],
    [
     "3D",
     "#f7e916",
     "#0f0f0f"
    ],
    [
     "LAYOUT",
     "#0072b0",
     "#ffffff"
    ],
    [
     "2D",
     "#f7e916",
     "#0f0f0f"
    ]
   ],
   "meta": "CLIENT: COLORBABY · 2023 CALENDAR",
   "imagen": {
    "src": "img/caso/colorbaby/01.jpg",
    "ar": 1
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#e30613",
   "ink": "#ffffff",
   "sep": "●",
   "voces": [
    "Colorbaby",
    "2023",
    "2D + 3D"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE BRIEF",
   "epInk": "#003c8c",
   "tit": [
    "2023 calendar by Colorbaby."
   ],
   "txt": [
    "Colorbaby's 2023 calendar: a mix of 3D and hand illustration."
   ],
   "meta": [
    [
     "CLIENT",
     "Colorbaby"
    ],
    [
     "YEAR",
     "2023"
    ],
    [
     "TECHNIQUE",
     "3D + 2D drawing"
    ],
    [
     "MY ROLE",
     "Illustration · Layout"
    ],
    [
     "DELIVERY",
     "Desk calendar"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / THE SIX SCENES",
    "epInk": "#e30613",
    "bg": "#f7e916",
    "ink": "#0f0f0f",
    "tit": [],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/colorbaby/02.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/colorbaby/03.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/colorbaby/04.jpg",
          "ar": 1
         }
        ]
       },
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/colorbaby/05.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/colorbaby/06.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/colorbaby/07.jpg",
          "ar": 1
         }
        ]
       }
      ]
     },
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/colorbaby/08.jpg",
          "ar": 7.3708
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / THE PAGE",
    "epInk": "#f7e916",
    "bg": "#0072b0",
    "ink": "#ffffff",
    "tit": [],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "minmax(0,560px)",
        "piezas": [
         {
          "src": "img/caso/colorbaby/09.jpg",
          "pie": "JANUARY",
          "ar": 1
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / 2D OVER 3D",
    "epInk": "#f7e916",
    "bg": "#e30613",
    "ink": "#ffffff",
    "tit": [
     "A mix of 3D and hand illustration"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/colorbaby/10.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/colorbaby/11.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/colorbaby/12.jpg",
          "ar": 1
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#0072b0",
   "ink": "#f7e916",
   "tools": [
    "ILLUSTRATOR",
    "CINEMA 4D",
    "PHOTOSHOP",
    "INDESIGN"
   ],
   "rotulo": "Colorbaby",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "hero-solo-bio"
 },
 "pummba": {
  "titulo": "Pummba",
  "hero": {
   "bg": "#fffaf4",
   "ink": "#264b66",
   "tituloInk": "#264b66",
   "titulo": "Pummba",
   "sub": [
    "The homepage of a modular sofa brand. One sofa, six living rooms."
   ],
   "pills": [
    [
     "ILLUSTRATION",
     "#264b66",
     "#ffffff"
    ],
    [
     "ANIMATION",
     "#f37543",
     "#264b66"
    ],
    [
     "3D + 2D",
     "#264b66",
     "#ffffff"
    ],
    [
     "AFTER EFFECTS",
     "#f37543",
     "#264b66"
    ]
   ],
   "meta": "CLIENT: PUMMBA · AT STORIA CONTENIDOS",
   "imagen": {
    "src": "img/caso/pummba/01.jpg",
    "ar": 1.5707
   }
  },
  "marquee": {
   "bg": "#fffaf4",
   "banda": "#f37543",
   "ink": "#fffaf4",
   "sep": "●",
   "voces": [
    "Pummba",
    "One sofa, six living rooms",
    "3D + 2D"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE BRIEF",
   "epInk": "#003c8c",
   "tit": [
    "The sofa doesn't change. It just changes places."
   ],
   "txt": [
    "Illustration and animation for Pummba's website."
   ],
   "meta": [
    [
     "CLIENT",
     "Pummba"
    ],
    [
     "STUDIO",
     "Storia Contenidos"
    ],
    [
     "FORMAT",
     "16:9"
    ],
    [
     "TECHNIQUE",
     "3D + 2D in After Effects"
    ],
    [
     "MY ROLE",
     "Illustration · Animation"
    ]
   ]
  },
  "secciones": [
   {
    "ep": "01 / SIX LIVING ROOMS",
    "epInk": "#f37543",
    "bg": "#fffaf4",
    "ink": "#264b66",
    "tit": [
     "The same pieces, six ways to use them"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/pummba/02.jpg",
          "ar": 1.5721
         },
         {
          "src": "img/caso/pummba/03.jpg",
          "ar": 1.5721
         }
        ]
       },
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/pummba/04.jpg",
          "ar": 1.5721
         },
         {
          "src": "img/caso/pummba/05.jpg",
          "ar": 1.5721
         }
        ]
       },
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/pummba/06.jpg",
          "ar": 1.5721
         },
         {
          "src": "img/caso/pummba/07.jpg",
          "ar": 1.5721
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / THE LOOP",
    "epInk": "#f37543",
    "bg": "#264b66",
    "ink": "#fffaf4",
    "tit": [
     "A fixed camera and everything else moving"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "ar": 1.5731,
          "poster": "img/caso/pummba/08.jpg",
          "video": "img/caso/video/pummba-home.mp4"
         }
        ]
       }
      ]
     },
     {
      "tipo": "rotulo",
      "txt": "FRAMES"
     },
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "repeat(5,1fr)",
        "piezas": [
         {
          "src": "img/caso/pummba/09.jpg",
          "ar": 1.5696
         },
         {
          "src": "img/caso/pummba/10.jpg",
          "ar": 1.5696
         },
         {
          "src": "img/caso/pummba/11.jpg",
          "ar": 1.5696
         },
         {
          "src": "img/caso/pummba/12.jpg",
          "ar": 1.5696
         },
         {
          "src": "img/caso/pummba/13.jpg",
          "ar": 1.5696
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / DETAIL",
    "epInk": "#fffaf4",
    "bg": "#f37543",
    "ink": "#ffffff",
    "tit": [],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/pummba/14.jpg",
          "ar": 0.9702
         },
         {
          "src": "img/caso/pummba/15.jpg",
          "ar": 0.9702
         },
         {
          "src": "img/caso/pummba/16.jpg",
          "ar": 0.9702
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#264b66",
   "ink": "#f37543",
   "tools": [
    "AFTER EFFECTS",
    "CINEMA 4D",
    "ILLUSTRATOR",
    "ILLUSTRATION"
   ],
   "rotulo": "Pummba",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "colorbaby"
 },
 "tashi": {
  "titulo": "Tashi",
  "hero": {
   "bg": "#ff80a9",
   "ink": "#160c53",
   "titulo": "Tashi",
   "tituloInk": "#160c53",
   "sub": [
    "Character design for the Master's in Editorial Illustration at Escuela Trazos."
   ],
   "pills": [
    [
     "CHARACTER",
     "#160c53",
     "#ffffff"
    ],
    [
     "ILLUSTRATION",
     "#7fd4c4",
     "#160c53"
    ],
    [
     "EXPRESSIONS",
     "#160c53",
     "#ffffff"
    ],
    [
     "ESCUELA TRAZOS",
     "#7fd4c4",
     "#160c53"
    ]
   ],
   "meta": "MASTER'S IN EDITORIAL ILLUSTRATION · ESCUELA TRAZOS",
   "imagen": {
    "src": "img/caso/tashi/01.jpg",
    "ar": 0.5978
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#fe374c",
   "ink": "#ffffff",
   "sep": "●",
   "voces": [
    "Tashi",
    "Milk cookies",
    "Bad temper"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE STORY",
   "epInk": "#003c8c",
   "tit": [
    "Her cookies are legendary. She isn't."
   ],
   "txt": [
    "Character design for the Master's in Editorial Illustration at Escuela Trazos. Tashi is a girl who sells milk cookies from a small wooden stall on the edge of a mountain in Tibet."
   ],
   "meta": [
    [
     "TYPE",
     "Master's in Editorial Illustration · Escuela Trazos"
    ],
    [
     "CHARACTER",
     "Tashi"
    ],
    [
     "TECHNIQUE",
     "Digital illustration"
    ],
    [
     "MY ROLE",
     "Character design · Illustration"
    ],
    [
     "DELIVERY",
     "Final illustration + expression sheet"
    ]
   ],
   "despues": false
  },
  "secciones": [
   {
    "ep": "01 / THE ILLUSTRATION",
    "epInk": "#fe374c",
    "bg": "#f7e8d1",
    "ink": "#160c53",
    "tit": [
     "The cookie, like a halo"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "560fr 712fr",
        "piezas": [
         {
          "src": "img/caso/tashi/02.jpg",
          "ar": 0.5977
         },
         {
          "panel": {
           "bg": "#7fd4c4",
           "ink": "#160c53",
           "ep": "THE CONTRADICTION",
           "tit": [
            "The shape is sweet. The look isn't."
           ],
           "txt": "Pink pom-pom hat, earmuffs, mint earrings and round cheeks. Everything about her looks friendly until you reach the eyes."
          }
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / EMOTIONS",
    "epInk": "#ffffff",
    "bg": "#ff80a9",
    "ink": "#160c53",
    "tit": [
     "Different emotions"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/tashi/03.jpg",
          "ar": 0.9937
         },
         {
          "src": "img/caso/tashi/04.jpg",
          "ar": 0.9937
         },
         {
          "src": "img/caso/tashi/05.jpg",
          "ar": 0.9937
         },
         {
          "src": "img/caso/tashi/06.jpg",
          "ar": 0.9937
         }
        ]
       }
      ]
     },
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "src": "img/caso/tashi/07.jpg",
          "ar": 1.7778
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / THE SKETCH",
    "epInk": "#fe374c",
    "bg": "#ffffff",
    "ink": "#160c53",
    "tit": [],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/tashi/08.jpg",
          "ar": 0.8821
         },
         {
          "src": "img/caso/tashi/09.jpg",
          "ar": 0.8821
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#160c53",
   "ink": "#ff80a9",
   "tools": [
    "PHOTOSHOP",
    "ILLUSTRATION",
    "CHARACTER",
    "PERSONAL"
   ],
   "rotulo": "Tashi",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "pummba"
 },
 "la-vanguardia": {
  "titulo": "La Vanguardia",
  "hero": {
   "bg": "#041c4a",
   "ink": "#ffffff",
   "titulo": "La Vanguardia",
   "tituloInk": "#ffffff",
   "sub": [
    "“Porque te interesa”. Two pieces: the campaign and what it achieved."
   ],
   "pills": [
    [
     "EDITING",
     "#ffffff",
     "#041c4a"
    ],
    [
     "POST-PRODUCTION",
     "#ffe500",
     "#041c4a"
    ],
    [
     "SOUND DESIGN",
     "#ffffff",
     "#041c4a"
    ],
    [
     "CASANOVA",
     "#ffe500",
     "#041c4a"
    ]
   ],
   "meta": "CLIENT: LA VANGUARDIA · AT CASANOVA",
   "imagen": {
    "src": "img/caso/la-vanguardia/01.jpg",
    "ar": 1.7751
   }
  },
  "marquee": {
   "bg": "#ffffff",
   "banda": "#041c4a",
   "ink": "#ffffff",
   "sep": "●",
   "voces": [
    "Porque te interesa",
    "La Vanguardia",
    "Editing and sound"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE BRIEF",
   "epInk": "#003c8c",
   "tit": [
    "Two videos. Two ways of telling the same campaign."
   ],
   "txt": [
    "Videos for “Porque te interesa”, La Vanguardia's campaign."
   ],
   "meta": [
    [
     "CLIENT",
     "La Vanguardia"
    ],
    [
     "AGENCY",
     "Casanova"
    ],
    [
     "CAMPAIGN",
     "“Porque te interesa”"
    ],
    [
     "MY ROLE",
     "Editing · Post-production · Sound design"
    ],
    [
     "DELIVERY",
     "2 videos · 20 s and 1:28"
    ]
   ],
   "despues": false
  },
  "secciones": [
   {
    "ep": "01 / THE TWO PIECES",
    "epInk": "#041c4a",
    "bg": "#f2f1ed",
    "ink": "#041c4a",
    "tit": [],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "ar": 1.7778,
          "poster": "img/caso/la-vanguardia/02.jpg",
          "video": "img/caso/video/lv-spot.mp4"
         }
        ]
       },
       {
        "cols": "1fr",
        "piezas": [
         {
          "ar": 1.7778,
          "poster": "img/caso/la-vanguardia/03.jpg",
          "video": "img/caso/video/lv-resumen.mp4"
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / HOW IT'S EDITED",
    "epInk": "#ffe500",
    "bg": "#041c4a",
    "ink": "#ffffff",
    "tit": [],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/la-vanguardia/04.jpg",
          "ar": 1.7741
         },
         {
          "src": "img/caso/la-vanguardia/05.jpg",
          "ar": 1.7741
         },
         {
          "src": "img/caso/la-vanguardia/06.jpg",
          "ar": 1.7741
         }
        ]
       },
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/la-vanguardia/07.jpg",
          "ar": 1.7796
         },
         {
          "src": "img/caso/la-vanguardia/08.jpg",
          "ar": 1.7796
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#041c4a",
   "ink": "#ffe500",
   "tools": [
    "PREMIERE",
    "AFTER EFFECTS",
    "EDITING",
    "AUDIO"
   ],
   "rotulo": "La Vanguardia",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "tashi"
 },
 "ilustraciones": {
  "titulo": "Illustrations",
  "hero": {
   "bg": "#efede6",
   "ink": "#14141a",
   "titulo": "Illustrations",
   "tituloInk": "#14141a",
   "sub": [
    "Assorted illustrations."
   ],
   "pills": [
    [
     "POSTER",
     "#1e7a3c",
     "#ffffff"
    ],
    [
     "INFOGRAPHIC",
     "#101a3d",
     "#ffffff"
    ],
    [
     "GRAPHICS",
     "#e8479a",
     "#ffffff"
    ],
    [
     "EDITORIAL",
     "#7ec8f0",
     "#ffffff"
    ]
   ],
   "meta": "ASSORTED ILLUSTRATIONS",
   "imagen": {
    "src": "img/caso/ilustraciones/01.jpg",
    "ar": 0.7068
   }
  },
  "marquee": {
   "bg": "#efede6",
   "banda": "#14141a",
   "ink": "#efede6",
   "sep": "●",
   "voces": [
    "Illustrations",
    "Posters",
    "Infographics"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE COLLECTION",
   "epInk": "#003c8c",
   "tit": [
    "Assorted illustrations."
   ],
   "txt": [
    "Posters, infographics, graphics and a book cover."
   ],
   "meta": [
    [
     "TYPE",
     "Standalone pieces"
    ],
    [
     "PIECES",
     "9"
    ],
    [
     "FORMATS",
     "Poster · Infographic · Graphics · Editorial"
    ],
    [
     "MY ROLE",
     "Concept · Illustration · Design"
    ],
    [
     "TECHNIQUES",
     "Linework, digital painting, lettering, data"
    ]
   ],
   "despues": false
  },
  "secciones": [
   {
    "ep": "01 / POSTERS",
    "epInk": "#1e7a3c",
    "bg": "#efede6",
    "ink": "#14141a",
    "tit": [
     "Posters"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "595fr 691fr",
        "piezas": [
         {
          "src": "img/caso/ilustraciones/02.jpg",
          "ar": 0.7067
         },
         {
          "src": "img/caso/ilustraciones/03.jpg",
          "ar": 0.8207
         }
        ]
       },
       {
        "cols": "560fr 726fr",
        "piezas": [
         {
          "src": "img/caso/ilustraciones/04.jpg",
          "ar": 0.7273
         },
         {
          "src": "img/caso/ilustraciones/05.jpg",
          "ar": 0.9429
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / INFOGRAPHICS",
    "epInk": "#ffd400",
    "bg": "#101a3d",
    "ink": "#ffffff",
    "tit": [
     "Infographics"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/ilustraciones/06.jpg",
          "ar": 0.7074
         },
         {
          "src": "img/caso/ilustraciones/07.jpg",
          "ar": 0.7074
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / GRAPHICS",
    "epInk": "#ffffff",
    "bg": "#e8479a",
    "ink": "#14141a",
    "tit": [
     "Graphics"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/ilustraciones/08.jpg",
          "ar": 1
         },
         {
          "src": "img/caso/ilustraciones/09.jpg",
          "ar": 1
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "04 / EDITORIAL",
    "epInk": "#ffffff",
    "bg": "#7ec8f0",
    "ink": "#14141a",
    "tit": [
     "Editorial"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "575fr 711fr",
        "piezas": [
         {
          "src": "img/caso/ilustraciones/10.jpg",
          "ar": 0.7215
         },
         {
          "src": "img/caso/ilustraciones/11.jpg",
          "ar": 0.8921
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#14141a",
   "ink": "#efede6",
   "tools": [
    "ILLUSTRATOR",
    "PHOTOSHOP",
    "PROCREATE",
    "INDESIGN"
   ],
   "rotulo": "Illustrations",
   "rotuloInk": "#ffffff"
  },
  "siguiente": "la-vanguardia"
 },
 "kalk": {
  "titulo": "Kalk",
  "hero": {
   "bg": "#f4f4f2",
   "ink": "#101010",
   "titulo": "Kalk",
   "tituloInk": "#101010",
   "sub": [
    "3D-modelled jewellery for performance ads."
   ],
   "pills": [
    [
     "3D",
     "#101010",
     "#ffffff"
    ],
    [
     "MODELLING",
     "#c9a227",
     "#101010"
    ],
    [
     "ANIMATION",
     "#101010",
     "#ffffff"
    ],
    [
     "PERFORMANCE",
     "#c9a227",
     "#101010"
    ]
   ],
   "meta": "CLIENT: KALK · PERFORMANCE ADS",
   "imagen": {
    "src": "img/caso/kalk/01.jpg",
    "ar": 0.5626
   }
  },
  "marquee": {
   "bg": "#f4f4f2",
   "banda": "#101010",
   "ink": "#f4f4f2",
   "sep": "●",
   "voces": [
    "Kalk",
    "Gold, silver, emerald",
    "3D"
   ],
   "giro": 2.4
  },
  "brief": {
   "bg": "#ffffff",
   "ink": "#0a1a2e",
   "ep": "THE BRIEF",
   "epInk": "#003c8c",
   "tit": [
    "A small piece of jewellery that has to fill the screen."
   ],
   "txt": [
    "Concept for Kalk: 3D modelling, animation and illustration to show the brand's pieces in performance ads."
   ],
   "meta": [
    [
     "CLIENT",
     "Kalk"
    ],
    [
     "FORMAT",
     "9:16 · 1080×1920"
    ],
    [
     "TECHNIQUE",
     "3D modelling and animation"
    ],
    [
     "MY ROLE",
     "Concept · 3D · Animation"
    ],
    [
     "DELIVERY",
     "3 performance ads"
    ]
   ],
   "despues": false
  },
  "secciones": [
   {
    "ep": "01 / THE THREE PIECES",
    "epInk": "#c9a227",
    "bg": "#f4f4f2",
    "ink": "#101010",
    "tit": [
     "Gold, silver and a raw emerald"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr",
        "piezas": [
         {
          "ar": 0.5622,
          "poster": "img/caso/kalk/02.jpg",
          "video": "img/caso/video/kalk-oro.mp4"
         },
         {
          "ar": 0.5622,
          "poster": "img/caso/kalk/03.jpg",
          "video": "img/caso/video/kalk-plata.mp4"
         },
         {
          "ar": 0.5622,
          "poster": "img/caso/kalk/04.jpg",
          "video": "img/caso/video/kalk-esmeralda.mp4"
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "02 / THE SYSTEM",
    "epInk": "#c9a227",
    "bg": "#101010",
    "ink": "#ffffff",
    "tit": [
     "Lots of white, very little else"
    ],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr 1fr 1fr 1fr",
        "piezas": [
         {
          "src": "img/caso/kalk/05.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/kalk/06.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/kalk/07.jpg",
          "ar": 0.5629
         },
         {
          "src": "img/caso/kalk/08.jpg",
          "ar": 0.5629
         }
        ]
       }
      ]
     }
    ]
   },
   {
    "ep": "03 / THE MATERIALS",
    "epInk": "#c9a227",
    "bg": "#ffffff",
    "ink": "#101010",
    "tit": [],
    "txt": "",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "500fr 410fr 350fr",
        "piezas": [
         {
          "src": "img/caso/kalk/09.jpg",
          "ar": 1.4045
         },
         {
          "src": "img/caso/kalk/10.jpg",
          "ar": 1.1517
         },
         {
          "src": "img/caso/kalk/11.jpg",
          "ar": 0.9831
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#101010",
   "ink": "#c9a227",
   "tools": [
    "CINEMA 4D",
    "AFTER EFFECTS",
    "3D MODELLING",
    "PERFORMANCE"
   ],
   "rotulo": "Kalk",
   "rotuloInk": "#f4f4f2"
  },
  "siguiente": "ilustraciones"
 },
 "reel": {
  "titulo": "Reel",
  "hero": {
   "bg": "#A96BF3",
   "bgGrad": "linear-gradient(180deg,#8B3FE0 0%,#A96BF3 32%,#D69CF5 58%,#F0D9FB 80%,#FFFFFF 98%)",
   "ink": "#FFFFFF",
   "titulo": "Reel",
   "sub": [
    "A selection of the motion work from the last few years, cut together in 53 seconds."
   ],
   "pills": [
    [
     "MOTION GRAPHICS",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ],
    [
     "2D & 3D",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ],
    [
     "DIRECTION",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ],
    [
     "ANIMATION",
     "rgba(255,255,255,.18)",
     "#FFFFFF"
    ]
   ],
   "meta": "2026 · SHOWREEL",
   "imagen": {
    "src": "img/caso/reel/hero.jpg",
    "ar": "16/9"
   }
  },
  "marquee": {
   "bg": "#FFFFFF",
   "banda": "#8B3FE0",
   "ink": "#FFFFFF",
   "sep": "✦",
   "voces": [
    "Motion graphics",
    "2D",
    "3D",
    "Illustration",
    "Animation"
   ],
   "giro": -2.2
  },
  "secciones": [
   {
    "ep": "REEL",
    "epInk": "#8B3FE0",
    "bg": "#0E0A16",
    "ink": "#F3ECFF",
    "bloques": [
     {
      "tipo": "filas",
      "filas": [
       {
        "cols": "1fr",
        "piezas": [
         {
          "embed": true,
          "url": "https://player.vimeo.com/video/1123494146?badge=0&autopause=0&player_id=0&app_id=58479",
          "ar": "16/9"
         }
        ]
       }
      ]
     }
    ]
   }
  ],
  "cierre": {
   "bg": "#A96BF3",
   "bgGrad": "linear-gradient(180deg,#8B3FE0 0%,#A96BF3 32%,#D69CF5 58%,#F0D9FB 80%,#FFFFFF 98%)",
   "ink": "#FFFFFF",
   "rotulo": "Reel",
   "tools": [
    "AFTER EFFECTS",
    "CINEMA 4D",
    "ILLUSTRATOR",
    "MOTION GRAPHICS"
   ]
  },
  "siguiente": "victoria"
 }
};
